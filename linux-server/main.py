...
async def stream_screen(sid):
    # mss.mss() context manager is removed
    while True:
        try:
            # Check if the client is still connected by trying to get the session
            await sio.get_session(sid)

            # Note: pyscreenshot.grab() may still have issues on Wayland depending on backend config.
            # It will attempt to use 'gnome-screenshot' or 'spectacle' or xdg-desktop-portal
            # which might prompt the user.

            img = pyscreenshot.grab() # This directly returns a PIL Image object
